...
extern "C" void hyacinXAllGatherV1Dcol_bufferSize(int32_t M, int32_t comm_size, hyacinPrecision_t Atype, uint64_t* dev_work_bytes, uint64_t* pinned_work_bytes) {
  if (M <= 0) { *dev_work_bytes = *pinned_work_bytes = uint64_t(0); return; }

  int64_t Mi = int64_t(M);
  switch(Atype) {
    case HYACIN_F32: break;
    case HYACIN_F64: case HYACIN_F32_COMPLEX: Mi <<= 1; break; 
    case HYACIN_DD: case HYACIN_QF: case HYACIN_F64_COMPLEX: Mi <<= 2; break;
