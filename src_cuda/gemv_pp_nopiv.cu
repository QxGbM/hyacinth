...
__global__ void gemv_pp_nopiv_kernel(int32_t N, matrix_t sq, matrix_ptr A, int64_t lda, real_ptr D, idx_ptr idx) {
  ...
  for (int32_t i = block_offset + int32_t(threadIdx.x); i < N; i += elements) {
    real_t thread_c = D[i];
    pp_func(A[i], A[int64_t(i + 1) * lda - 1], thread_c);
    thread_x = cmp_max(thread_x, idx_t({ thread_c, i }));
    D[i] = thread_c;
  }
...
void internal::Cholesky::gemv_pp_nopiv_f64(cudaStream_t stream, int32_t M, int32_t N, double* sq, double* A, int32_t lda, double* D) {
  gemv_pp_nopiv_kernel <double, double* __restrict__, double, double* __restrict__, double_idx, double_idx* __restrict__, grid_blocks, block_threads>
    <<< grid_blocks, block_threads, 0, stream >>> (N - 1, *sq, &A[M + 1], lda, &D[1], (double_idx*)sq);
  imax_f64_host_sync(stream, N - 1, std::min(grid_blocks, (N + block_threads - 2) / block_threads), sq);
}

void internal::Cholesky::gemv_pp_nopiv_f32(cudaStream_t stream, int32_t M, int32_t N, float* sq, float* A, int32_t lda, float* D) {
  gemv_pp_nopiv_kernel <float, float* __restrict__, float, float* __restrict__, float_idx, float_idx* __restrict__, grid_blocks, block_threads>
    <<< grid_blocks, block_threads, 0, stream >>> (N - 1, *sq, &A[M + 1], lda, &D[1], (float_idx*)sq);
  imax_f32_host_sync(stream, N - 1, std::min(grid_blocks, (N + block_threads - 2) / block_threads), sq);
}
...
void internal::Cholesky::gemv_pp_nopiv_f128_dd(cudaStream_t stream, int32_t M, int32_t N, double2* sq, double2* A, int32_t lda, double2* D) {
  gemv_pp_nopiv_kernel <double2, double2* __restrict__, double2, double2* __restrict__, double2_idx, double2_idx* __restrict__, grid_blocks, block_threads>
    <<< grid_blocks, block_threads, 0, stream >>> (N - 1, *sq, &A[M + 1], lda, &D[1], (double2_idx*)sq);
  imax_f128_dd_host_sync(stream, N - 1, std::min(grid_blocks, (N + block_threads - 2) / block_threads), sq);
}
...
void internal::Cholesky::gemv_pp_nopiv_f128_qf(cudaStream_t stream, int32_t M, int32_t N, float4* sq, float4* A, int32_t lda, float4* D) {
  gemv_pp_nopiv_kernel <float4, float4* __restrict__, float4, float4* __restrict__, float4_idx, float4_idx* __restrict__, grid_blocks, block_threads>
    <<< grid_blocks, block_threads, 0, stream >>> (N - 1, *sq, &A[M + 1], lda, &D[1], (float4_idx*)sq);
  imax_f128_qf_host_sync(stream, N - 1, std::min(grid_blocks, (N + block_threads - 2) / block_threads), sq);
}
...
void internal::Cholesky::gemv_pp_nopiv_cf64(cudaStream_t stream, int32_t M, int32_t N, double* sq, std::complex<double>* A, int32_t lda, double* D) {
  ...
  gemv_pp_nopiv_kernel <double, double* __restrict__, cuDoubleComplex, cuDoubleComplex* __restrict__, double_idx, double_idx* __restrict__, grid_blocks, block_threads>
...
void internal::Cholesky::gemv_pp_nopiv_cf32(cudaStream_t stream, int32_t M, int32_t N, float* sq, std::complex<float>* A, int32_t lda, float* D) {
  ...
  gemv_pp_nopiv_kernel <float, float* __restrict__, cuComplex, cuComplex* __restrict__, float_idx, float_idx* __restrict__, grid_blocks, block_threads>
...
void internal::Cholesky::gemv_pp_nopiv_cf128_dd(cudaStream_t stream, int32_t M, int32_t N, double2* sq, complex_double2* A, int32_t lda, double2* D) {
  ...
  gemv_pp_nopiv_kernel <double2, double2* __restrict__, complex_double2, complex_double2* __restrict__, double2_idx, double2_idx* __restrict__, grid_blocks, block_threads>
...
void internal::Cholesky::gemv_pp_nopiv_cf128_qf(cudaStream_t stream, int32_t M, int32_t N, float4* sq, complex_float4* A, int32_t lda, float4* D) {
  ...
  gemv_pp_nopiv_kernel <float4, float4* __restrict__, complex_float4, complex_float4* __restrict__, float4_idx, float4_idx* __restrict__, grid_blocks, block_threads>
