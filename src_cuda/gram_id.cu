...
extern "C" int32_t hyacinXGinterp(hyacinHandle_t handle, char fillmode, double epi, int32_t N, int32_t K, int32_t p, hyacinPrecision_t Atype, void* X, int32_t ldx, int32_t* jpiv, hyacinPrecision_t Gtype, void* G, int32_t ldg) {
  ...
  switch (Gtype) {
    case HYACIN_F64:
      return diag_piv_dispatcher<double>(handle.cudaStream, handle.cublasHandle, fillmode, epi, N, K, p, Atype, jpiv, X, ldx, (double*)G, ldg, handle.pinnedWorkspace);
    case HYACIN_F32:
      return diag_piv_dispatcher<float>(handle.cudaStream, handle.cublasHandle, fillmode, epi, N, K, p, Atype, jpiv, X, ldx, (float*)G, ldg, handle.pinnedWorkspace);
    case HYACIN_DD:
      return diag_piv_dispatcher<double2>(handle.cudaStream, handle.cublasHandle, fillmode, epi, N, K, p, Atype, jpiv, X, ldx, (double2*)G, ldg, handle.pinnedWorkspace);
    case HYACIN_QF:
    ...
    case HYACIN_QF_COMPLEX:
      return diag_piv_dispatcher<float4>(handle.cudaStream, handle.cublasHandle, fillmode, epi, N, K, p, Atype, jpiv, X, ldx, (complex_float4*)G, ldg, handle.pinnedWorkspace);
    default: return 0;
  }
}
