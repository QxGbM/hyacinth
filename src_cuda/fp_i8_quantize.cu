...
template <int32_t order, class real_t, class matrix_t> struct quantize_func {
  ...
  __device__ __forceinline__ void quantize(double f, int32_t vec_e, uint32_t (&code)[ORDER]) {
    device::int8::quantize_double_align<device::Config::exp_base>(f, vec_e, code);
  }

  template <uint32_t ORDER>
  __device__ __forceinline__ void quantize(float f, int32_t vec_e, uint32_t (&code)[ORDER]) {
    device::int8::quantize_float_align<device::Config::exp_base>(f, vec_e, code);
  }
  ...
  __device__ __forceinline__ void operator()(int64_t i) {
    ...
    if constexpr(sizeof(real_t) < sizeof(matrix_t)) {
      ...
      quantize(A_i.x, expon, c.code);
      quantize(A_i.y, expon, c_im.code);
