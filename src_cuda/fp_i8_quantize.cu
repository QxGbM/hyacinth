...
__device__ __forceinline__ void quantize_f64_i8limbs(double x, int32_t expon, uint64_t lo, uint32_t hi, uint32_t (&code)[3]) {
  uint32_t c = 0; device::int8::quantize_f64_u32limbs(x, expon, lo, hi, code);
...
__device__ __forceinline__ void quantize_f64_i8rems(double x, int32_t expon, uint64_t lo, uint32_t hi, uint32_t (&code)[3]) {
  device::int8::quantize_f64_u32limbs(x, expon, lo, hi, code);
  constexpr uint32_t m_lo = uint32_t(MO), m_hi = uint32_t(MO >> 32);
  constexpr uint32_t r_lo = uint32_t(R32), r_hi = uint32_t(R32 >> 32);
  uint32_t c_lo = device::int8::conv_u32i8_modular<m_lo, r_lo>(code);
  uint32_t c_hi = device::int8::conv_u32i8_modular<m_hi, r_hi>(code);
  code[0] = c_lo; code[1] = c_hi;
}
...
__global__ void quantize_kernel(int64_t M, int64_t N, matrix_const_ptr A, int64_t lda, uint64_t lo, uint32_t hi, const uint64_t* __restrict__ vec_expon, int8_t* __restrict__ B, int64_t ldb, int64_t strideB) {
  ...
  if constexpr(op == 0)
    ...
    quantize_f64_i8rems<MO, R32>(double(A_i), expon, lo, hi, code);
  else if constexpr(op == 3)
    quantize_f64_i8rems<MO, R32>(double(A_i.x), expon, lo, hi, code);
  ...
  if constexpr(op & 1) {
    if constexpr(op == 1)
      ...
      quantize_f64_i8rems<MO, R32>(double(A_i.y), expon, lo, hi, code);
...
inline void quantize_dispatcher(cudaStream_t stream, int64_t M, int64_t N, matrix_const_ptr C, int64_t ldc, int32_t umax, const uint64_t* vec_expon, int32_t orderA, int8_t* A, int64_t lda) {
  ...
  switch (orderA) {
    case 1: quantize_kernel<1, MO, R32, matrix_t, matrix_const_ptr, op> <<< grid, block_threads, 0, stream >>> (M, N, C, ldc, lo, hi, vec_expon, A, lda, strideA); break;
    case 2: quantize_kernel<2, MO, R32, matrix_t, matrix_const_ptr, op> <<< grid, block_threads, 0, stream >>> (M, N, C, ldc, lo, hi, vec_expon, A, lda, strideA); break;
    case 3: quantize_kernel<3, MO, R32, matrix_t, matrix_const_ptr, op> <<< grid, block_threads, 0, stream >>> (M, N, C, ldc, lo, hi, vec_expon, A, lda, strideA); break;
    case 4: quantize_kernel<4, MO, R32, matrix_t, matrix_const_ptr, op> <<< grid, block_threads, 0, stream >>> (M, N, C, ldc, lo, hi, vec_expon, A, lda, strideA); break;
    case 5: quantize_kernel<5, MO, R32, matrix_t, matrix_const_ptr, op> <<< grid, block_threads, 0, stream >>> (M, N, C, ldc, lo, hi, vec_expon, A, lda, strideA); break;
    case 6: quantize_kernel<6, MO, R32, matrix_t, matrix_const_ptr, op> <<< grid, block_threads, 0, stream >>> (M, N, C, ldc, lo, hi, vec_expon, A, lda, strideA); break;
    case 7: quantize_kernel<7, MO, R32, matrix_t, matrix_const_ptr, op> <<< grid, block_threads, 0, stream >>> (M, N, C, ldc, lo, hi, vec_expon, A, lda, strideA); break;
    case 8: quantize_kernel<8, MO, R32, matrix_t, matrix_const_ptr, op> <<< grid, block_threads, 0, stream >>> (M, N, C, ldc, lo, hi, vec_expon, A, lda, strideA); break;
    case 9: quantize_kernel<9, MO, R32, matrix_t, matrix_const_ptr, op> <<< grid, block_threads, 0, stream >>> (M, N, C, ldc, lo, hi, vec_expon, A, lda, strideA); break;
    case 10: quantize_kernel<10, MO, R32, matrix_t, matrix_const_ptr, op> <<< grid, block_threads, 0, stream >>> (M, N, C, ldc, lo, hi, vec_expon, A, lda, strideA); break;
    case 11: quantize_kernel<11, MO, R32, matrix_t, matrix_const_ptr, op> <<< grid, block_threads, 0, stream >>> (M, N, C, ldc, lo, hi, vec_expon, A, lda, strideA); break;
    default: break;
  }
}
