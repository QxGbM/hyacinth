...
template <class real_t> struct __align__(sizeof(real_t) << 1) real_pair { real_t real; int32_t idx; };
...
__global__ void imax_kernel(int32_t N, real_const_ptr X, real_pair<real_t>* __restrict__ pair) {
  ...
  if (threadIdx.x == 0 && block_res.idx < N)
    ...
    pair[0] = real_pair<real_t>({ real_t(), -1 });
}
...
inline void imax_host(int32_t N, int32_t lenx, real_pair<real_t>* x) {
  real_pair<real_t> init({ real_t(), -1 });
  real_pair<real_t> res = std::reduce(x, &x[lenx], init, real_pair_max<real_t>());
  x[0] = res.idx < N ? res : init;
}
...
void internal::Cholesky::imax_f64(cudaStream_t stream, int32_t N, double* X, double* diag_piv) {
  ...
  real_pair<double>* p = (real_pair<double>*)diag_piv;
  imax_kernel <double, double* __restrict__, const double* __restrict__, block_threads, items_per_thread>
    <<< 1, block_threads, 0, stream >>> (N, X, p);
...
void internal::Cholesky::imax_f32(cudaStream_t stream, int32_t N, float* X, float* diag_piv) {
  ...
  real_pair<float>* p = (real_pair<float>*)diag_piv;
  imax_kernel <float, float* __restrict__, const float* __restrict__, block_threads, items_per_thread>
    <<< 1, block_threads, 0, stream >>> (N, X, p);
  cudaStreamSynchronize(stream);
  imax_host(N, 1, p);
}
...
void internal::Cholesky::imax_f128_dd(cudaStream_t stream, int32_t N, double2* X, double2* diag_piv) {
  ...
  real_pair<double2>* p = (real_pair<double2>*)diag_piv;
  imax_kernel <double2, double2* __restrict__, const double2* __restrict__, block_threads, items_per_thread>
    <<< 1, block_threads, 0, stream >>> (N, X, p);
  cudaStreamSynchronize(stream);
  imax_host(N, 1, p);
}
...
void internal::Cholesky::imax_f128_qf(cudaStream_t stream, int32_t N, float4* X, float4* diag_piv) {
  ...
  real_pair<float4>* p = (real_pair<float4>*)diag_piv;
  imax_kernel <float4, float4* __restrict__, const float4* __restrict__, block_threads, items_per_thread>
    <<< 1, block_threads, 0, stream >>> (N, X, p);
  cudaStreamSynchronize(stream);
  imax_host(N, 1, p);
}
