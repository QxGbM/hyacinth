...
__global__ void quantize_crt_kernel(int64_t M, const matrix_t* __restrict__ A, int64_t lda, uint32_t corr, const int32_t* __restrict__ vexp, int8_t* __restrict__ B, int64_t ldb, int64_t strideB, sum_t* __restrict__ vsum) {
  ...
  if (expon == int_max) {
    ...
    if (int32_t(threadIdx.x) == 0) { conv_acc(threadA, M, corr, vsum); }
  } else {
...
inline void quantize_crt_dispatcher(cudaStream_t stream, int32_t M, int32_t N, int32_t orderA, const matrix_t* A, int32_t lda, uint32_t corr, const int32_t* vexp, int8_t* B, int32_t ldb, sum_t* vsum) {
  ...
  switch (orderA) {
    case 2: quantize_crt_kernel<2, block_threads> <<< N, block_threads, 0, stream >>> (M64, A, lda64, corr, vexp, B, ldb64, strideB, vsum); return;
    case 3: quantize_crt_kernel<3, block_threads> <<< N, block_threads, 0, stream >>> (M64, A, lda64, corr, vexp, B, ldb64, strideB, vsum); return;
    case 4: quantize_crt_kernel<4, block_threads> <<< N, block_threads, 0, stream >>> (M64, A, lda64, corr, vexp, B, ldb64, strideB, vsum); return;
    case 5: quantize_crt_kernel<5, block_threads> <<< N, block_threads, 0, stream >>> (M64, A, lda64, corr, vexp, B, ldb64, strideB, vsum); return;
    case 6: quantize_crt_kernel<6, block_threads> <<< N, block_threads, 0, stream >>> (M64, A, lda64, corr, vexp, B, ldb64, strideB, vsum); return;
    case 7: quantize_crt_kernel<7, block_threads> <<< N, block_threads, 0, stream >>> (M64, A, lda64, corr, vexp, B, ldb64, strideB, vsum); return;
    case 8: quantize_crt_kernel<8, block_threads> <<< N, block_threads, 0, stream >>> (M64, A, lda64, corr, vexp, B, ldb64, strideB, vsum); return;
    case 9: quantize_crt_kernel<9, block_threads> <<< N, block_threads, 0, stream >>> (M64, A, lda64, corr, vexp, B, ldb64, strideB, vsum); return;
    case 10: quantize_crt_kernel<10, block_threads> <<< N, block_threads, 0, stream >>> (M64, A, lda64, corr, vexp, B, ldb64, strideB, vsum); return;
    case 11: quantize_crt_kernel<11, block_threads> <<< N, block_threads, 0, stream >>> (M64, A, lda64, corr, vexp, B, ldb64, strideB, vsum); return;
    case 12: quantize_crt_kernel<12, block_threads> <<< N, block_threads, 0, stream >>> (M64, A, lda64, corr, vexp, B, ldb64, strideB, vsum); return;
    case 13: quantize_crt_kernel<13, block_threads> <<< N, block_threads, 0, stream >>> (M64, A, lda64, corr, vexp, B, ldb64, strideB, vsum); return;
    case 14: quantize_crt_kernel<14, block_threads> <<< N, block_threads, 0, stream >>> (M64, A, lda64, corr, vexp, B, ldb64, strideB, vsum); return;
    case 15: quantize_crt_kernel<15, block_threads> <<< N, block_threads, 0, stream >>> (M64, A, lda64, corr, vexp, B, ldb64, strideB, vsum); return;
    case 16: quantize_crt_kernel<16, block_threads> <<< N, block_threads, 0, stream >>> (M64, A, lda64, corr, vexp, B, ldb64, strideB, vsum); return;
    case 17: quantize_crt_kernel<17, block_threads> <<< N, block_threads, 0, stream >>> (M64, A, lda64, corr, vexp, B, ldb64, strideB, vsum); return;
    case 18: quantize_crt_kernel<18, block_threads> <<< N, block_threads, 0, stream >>> (M64, A, lda64, corr, vexp, B, ldb64, strideB, vsum); return;
    case 19: quantize_crt_kernel<19, block_threads> <<< N, block_threads, 0, stream >>> (M64, A, lda64, corr, vexp, B, ldb64, strideB, vsum); return;
    case 20: quantize_crt_kernel<20, block_threads> <<< N, block_threads, 0, stream >>> (M64, A, lda64, corr, vexp, B, ldb64, strideB, vsum); return;
    case 21: quantize_crt_kernel<21, block_threads> <<< N, block_threads, 0, stream >>> (M64, A, lda64, corr, vexp, B, ldb64, strideB, vsum); return;
    case 22: quantize_crt_kernel<22, block_threads> <<< N, block_threads, 0, stream >>> (M64, A, lda64, corr, vexp, B, ldb64, strideB, vsum); return;
    case 23: quantize_crt_kernel<23, block_threads> <<< N, block_threads, 0, stream >>> (M64, A, lda64, corr, vexp, B, ldb64, strideB, vsum); return;
    default: return;
  }
}
