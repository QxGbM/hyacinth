...
__global__ void decode_kernel(uint64_t M, int32_t N, int32_t expon, real_ptr A, int32_t split_k, uint64_t strideA, const int32_t* __restrict__ B) {
  ...
  int32_t threadA[ITEMS_PER_THREAD], threadC[ITEMS_PER_THREAD];
  ...
  for (uint64_t row = (blockIdx.x * elements_block); row < M; row += elements) {
    ...
    for (int32_t col = 0; col < N; ++col) {
      ...
      for (int32_t k = 1; k < 4; ++k) {
        if (k < split_k) {
          uint64_t A_k = A_col + uint64_t(k) * strideA;
          block_load.Load(&B[A_k], threadC, num_items);
...
inline void decode_dispatcher(cudaStream_t stream, int32_t order_lo, int32_t order_hi, int32_t order_k, int32_t split_k, int32_t N, real_ptr A, const int32_t* B, int32_t ld) {
  constexpr int32_t acc_bits = 31;
  int32_t order = order_hi - order_lo;
  int32_t acc_order = ((order + 1) * device::Config::exp_base + order_k + (acc_bits - 1)) / acc_bits;
  ...
  switch (acc_order) {
    case 1: decode_kernel <real_t, real_ptr, grid_blocks, block_threads, items_per_thread, device::Config::exp_base, 1>
      <<< grid_blocks, block_threads, 0, stream >>> (M, order, expon, A, split_k, strideA, B); break;
...
void internal::int8::decode_f64_strided_i32(cudaStream_t stream, int32_t order_lo, int32_t order_hi, int32_t order_k, int32_t split_k, int32_t N, double* A, const int32_t* B, int32_t ld) {
  decode_dispatcher<double, double* __restrict__>(stream, order_lo, order_hi, order_k, split_k, N, A, B, ld);
}

void internal::int8::decode_f32_strided_i32(cudaStream_t stream, int32_t order_lo, int32_t order_hi, int32_t order_k, int32_t split_k, int32_t N, float* A, const int32_t* B, int32_t ld) {
  decode_dispatcher<float, float* __restrict__>(stream, order_lo, order_hi, order_k, split_k, N, A, B, ld);
}

void internal::int8::decode_f128_dd_strided_i32(cudaStream_t stream, int32_t order_lo, int32_t order_hi, int32_t order_k, int32_t split_k, int32_t N, double2* A, const int32_t* B, int32_t ld) {
  decode_dispatcher<double2, double2* __restrict__>(stream, order_lo, order_hi, order_k, split_k, N, A, B, ld);
}

void internal::int8::decode_f128_qf_strided_i32(cudaStream_t stream, int32_t order_lo, int32_t order_hi, int32_t order_k, int32_t split_k, int32_t N, float4* A, const int32_t* B, int32_t ld) {
  decode_dispatcher<float4, float4* __restrict__>(stream, order_lo, order_hi, order_k, split_k, N, A, B, ld);
}
