...
template <class real_t, int32_t ITEMS_PER_THREAD, int32_t REAL_ITEMS>
__device__ __forceinline__ void decode_i32_array(real_t (&val)[REAL_ITEMS], int32_t const (&c)[REAL_ITEMS], int32_t const (&expon)[ITEMS_PER_THREAD], int32_t expon_k) {
  acc_i32 acc_f;
  if constexpr(REAL_ITEMS == ITEMS_PER_THREAD * 2) {
    #pragma unroll
    for (int32_t i = 0; i < ITEMS_PER_THREAD; ++i) {
      int32_t e = expon[i] + expon_k;
      acc_f(val[i + i], c[i], e);
      acc_f(val[1 + i + i], c[i + ITEMS_PER_THREAD], e);
    }
  }
  else {
    #pragma unroll
...
__global__ void decode_kernel(int32_t k_begin, int32_t k_len, int32_t N, const int32_t* __restrict__ vec_expon, const int32_t* __restrict__ A, int32_t lda, uint64_t strideA, int64_t imAoffset, real_ptr C, int32_t ldc) {
  ...
  __shared__ typename cub::BlockLoad<real_t, BLOCK_THREADS, COMPLEX * ITEMS_PER_THREAD>::TempStorage temp_load_real;
  __shared__ typename cub::BlockStore<real_t, BLOCK_THREADS, COMPLEX * ITEMS_PER_THREAD>::TempStorage temp_store;

  cub::BlockLoad<int32_t, BLOCK_THREADS, ITEMS_PER_THREAD> block_load(temp_load);
  cub::BlockLoad<real_t, BLOCK_THREADS, COMPLEX * ITEMS_PER_THREAD> block_load_real(temp_load_real);
  cub::BlockStore<real_t, BLOCK_THREADS, COMPLEX * ITEMS_PER_THREAD> block_store(temp_store);
  int32_t row_expon[ITEMS_PER_THREAD], c[COMPLEX * ITEMS_PER_THREAD];
  const int32_t* B = &A[imAoffset];
  real_t val[COMPLEX * ITEMS_PER_THREAD];
  ...
  for (int32_t col = blockIdx.x; col < N; col += GRID_BLOCKS) {
    ...
    for (int32_t i = 0; i < N; i += elements) {
      ...
      block_load_real.Load(&C_col[i * COMPLEX], val, num_items * COMPLEX);
      ...
      for (int32_t k = 0; k < k_len; ++k) {
        ...
        if constexpr(1 < COMPLEX)
          block_load.Load(&B[A_k], *(int32_t(*)[ITEMS_PER_THREAD])(&c[ITEMS_PER_THREAD]), num_items);
        decode_i32_array(val, c, row_expon, BASE * k);
      }

      block_store.Store(&C_col[i * COMPLEX], val, num_items * COMPLEX);
    }
  }
}
...
void internal::int8::decode_cf64_strided_i32(cudaStream_t stream, int32_t order_lo, int32_t order_hi, int32_t N, const int32_t* vec_expon, const int32_t* A, int32_t lda, std::complex<double>* C, int32_t ldc) {
  int32_t order = order_hi - order_lo;
  uint64_t strideA = uint64_t(N) * uint64_t(lda);
  decode_kernel <double, double* __restrict__, grid_blocks, block_threads, items_per_thread, device::Config::exp_base, 2>
    <<< grid_blocks, block_threads, 0, stream >>> (order_lo, order, N, vec_expon, A, lda, strideA, uint64_t(order) * strideA, (double*)C, 2 * ldc);
}
...
void internal::int8::decode_cf32_strided_i32(cudaStream_t stream, int32_t order_lo, int32_t order_hi, int32_t N, const int32_t* vec_expon, const int32_t* A, int32_t lda, std::complex<float>* C, int32_t ldc) {
  int32_t order = order_hi - order_lo;
  uint64_t strideA = uint64_t(N) * uint64_t(lda);
  decode_kernel <float, float* __restrict__, grid_blocks, block_threads, items_per_thread, device::Config::exp_base, 2>
    <<< grid_blocks, block_threads, 0, stream >>> (order_lo, order, N, vec_expon, A, lda, strideA, uint64_t(order) * strideA, (float*)C, 2 * ldc);
}
...
void internal::int8::decode_complex_dd_strided_i32(cudaStream_t stream, int32_t order_lo, int32_t order_hi, int32_t N, const int32_t* vec_expon, const int32_t* A, int32_t lda, complex_double2* C, int32_t ldc) {
  int32_t order = order_hi - order_lo;
  uint64_t strideA = uint64_t(N) * uint64_t(lda);
  decode_kernel <double2, double2* __restrict__, grid_blocks, block_threads, items_per_thread, device::Config::exp_base, 2>
    <<< grid_blocks, block_threads, 0, stream >>> (order_lo, order, N, vec_expon, A, lda, strideA, uint64_t(order) * strideA, (double2*)C, 2 * ldc);
}
...
void internal::int8::decode_complex_qf_strided_i32(cudaStream_t stream, int32_t order_lo, int32_t order_hi, int32_t N, const int32_t* vec_expon, const int32_t* A, int32_t lda, complex_float4* C, int32_t ldc) {
  int32_t order = order_hi - order_lo;
  uint64_t strideA = uint64_t(N) * uint64_t(lda);
  decode_kernel <float4, float4* __restrict__, grid_blocks, block_threads, items_per_thread, device::Config::exp_base, 2>
    <<< grid_blocks, block_threads, 0, stream >>> (order_lo, order, N, vec_expon, A, lda, strideA, uint64_t(order) * strideA, (float4*)C, 2 * ldc);
}
