...
__global__ void potrf_init_kernel(real_t epi, int32_t p, int32_t N, matrix_t* __restrict__ X, int64_t incx, int32_t* __restrict__ jpiv, real_t* __restrict__ D, idx_t* __restrict__ work) {
  ...
  if (BLOCK_THREADS == nthreads) {
    if (int32_t(threadIdx.x) == 0) { X[0] = real_sqrt<matrix_t>(D[N] = _mul(epi, thread_x.real), thread_x, rsq); work[0] = rsq; work[BLOCK_THREADS].idx = thread_x.idx - 1; }
  } else {
...
__global__ void potrf_iter_kernel(int32_t iterMax, int32_t N, matrix_t* __restrict__ A, int64_t lda, int32_t* __restrict__ jpiv, real_t* __restrict__ D, idx_t* __restrict__ work, int32_t* __restrict__ out) {
  ...
  auto grid = cooperative_groups::this_grid();
  const int32_t tid = int32_t(grid.thread_rank()) + 1, nthreads = int32_t(grid.num_threads());
  if (int32_t(threadIdx.x) == 0) { rsq = work[0]; j = work[BLOCK_THREADS].idx; }
  cooperative_groups::this_thread_block().sync();

  int32_t M = 0;
  while (1 < N && 0 <= rsq.idx && M < iterMax) {
    ...
    idx_t thread_x = idx_t();
    for (int32_t i = tid; i < N; i += nthreads) {
    ...
    if (0 < j) for (int32_t i = tid - (1 + M); i < 0; i += nthreads)
    ...
    if (BLOCK_THREADS == nthreads) {
      if (int32_t(threadIdx.x) == 0) {
        if (0 < j) { int32_t p = jpiv[0]; jpiv[0] = jpiv[j]; jpiv[j] = p; }
        *A = real_sqrt<matrix_t>(D[N], thread_x, rsq);
        j = thread_x.idx - 1;
      }
    } else {
      if (int32_t(threadIdx.x) == 0) { work[blockIdx.x] = thread_x; } else { thread_x = idx_t(); }
      ...
      if (int32_t(blockIdx.x) == 0) {
        ...
        if (int32_t(threadIdx.x) == 0) {
          if (0 < j) { int32_t p = jpiv[0]; jpiv[0] = jpiv[j]; jpiv[j] = p; }
          *A = real_sqrt<matrix_t>(D[N], thread_x, rsq);
          work[0] = rsq; work[BLOCK_THREADS].idx = thread_x.idx - 1;
        }
      }
      grid.sync();
      if (int32_t(threadIdx.x) == 0) { rsq = work[0]; j = work[BLOCK_THREADS].idx; }
...
inline int32_t potrfp_dispatcher(cudaStream_t stream, char fillmode, double epi, int32_t k, int32_t p, int32_t N, matrix_t* A, int64_t lda, int32_t* jpiv, real_t* D, int32_t* rank) {
  ...
  uint8_t* diag = &((uint8_t*)D)[65536]; int64_t incx = lda + int64_t(1);
  void* initArgs[]{ &epi_f, &p, &N, &A, &incx, &jpiv, &diag, &D };
