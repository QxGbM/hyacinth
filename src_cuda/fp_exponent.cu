...
{ if (a == 0.) { return int_max; } else { int32_t x; frexp(a, &x); if constexpr(sign) return e - x; else return e + x; }}
template <int32_t sign> __device__ __forceinline__ int32_t float_frexp(float a, int32_t e)
{ if (a == 0.f) { return int_max; } else { int32_t x; frexpf(a, &x); if constexpr(sign) return e - x; else return e + x; }}
...
__global__ void vector_exponent_kernel(int64_t M, const real_t* __restrict__ A, int64_t lda, int32_t umax, int32_t* __restrict__ vexp) {
  ...
  if (threadIdx.x == 0) { *vexp = float_frexp<1>(thread_data, umax); }
}
