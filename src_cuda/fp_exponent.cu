...
__global__ void vector_range_kernel(int32_t M, int32_t N, const matrix_t* __restrict__ A, int64_t lda, const int32_t* __restrict__ vexp, int32_t* __restrict__ vbuf, int32_t* __restrict__ out) {
  ...
  if (blockIdx.x == 0) {
    ...
    threadI = cub::BlockReduce<int32_t, BLOCK_THREADS>(temp_reduce[1]).Reduce(threadI, cmp);
...
namespace internal::int8 {
  ...
  void vector_exp_diff(cudaStream_t stream, int32_t N, int32_t* d, const int32_t* vexp)
  { constexpr int32_t block_threads = 512; exp_diff_reduct_kernel<block_threads> <<< 1, block_threads, 0, stream >>> (N, vexp, d); cudaStreamSynchronize(stream); }
