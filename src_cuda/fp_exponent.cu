...
__global__ void vector_exponent_kernel(int32_t q_range, int32_t M, real_const_ptr A, int64_t lda, int32_t* __restrict__ vec_expon) {
  ...
  if (threadIdx.x == 0)
    vec_expon[blockIdx.x] = fp_expon(threadB) - q_range;
}
...
void internal::int8::vexp_f64(cudaStream_t stream, int32_t q_range, int32_t M, int32_t N, const double* A, int32_t lda, int32_t* vec_expon) {
  vector_exponent_kernel <double, const double* __restrict__, block_threads>
    <<< N, block_threads, 0, stream >>> (q_range, M, A, lda, vec_expon);
}
