...
int32_t device::Cholesky::zpotrfp(cudaStream_t stream, int32_t N, std::complex<double>* A, int32_t lda, int32_t* ipiv) {
  ...
  double* scale = (double*)(&ipiv[algnN]), *diag = (double*)(&A[N * lda]);
  ...
  for (int32_t i = 0; i < N; ++i) {
    ...
    internal::Cholesky::imax_double_complex(stream, N - i, Aprev, &diag[i], &A[i * (lda + 1)], lda, pivot_i, scale);
...
int32_t device::Cholesky::cpotrfp(cudaStream_t stream, int32_t N, std::complex<float>* A, int32_t lda, int32_t* ipiv) {
  int32_t algnN = (N + 3) & (~3), *pivot_i = &ipiv[algnN + 4];
  float* scale = (float*)(&ipiv[algnN]), *diag = (float*)(&A[N * lda]);
  std::iota(ipiv, &ipiv[N], 1);
  ...
  for (int32_t i = 0; i < N; ++i) {
    ...
    internal::Cholesky::imax_float_complex(stream, N - i, Aprev, &diag[i], &A[i * (lda + 1)], lda, pivot_i, scale);
    cudaStreamSynchronize(stream);
...
int32_t device::Cholesky::complex_double_double_potrfp(cudaStream_t stream, int32_t N, complex_double2* A, int32_t lda, int32_t* ipiv) {
  int32_t algnN = (N + 3) & (~3), *pivot_i = &ipiv[algnN + 4];
  double2* scale = (double2*)(&ipiv[algnN]), *diag = (double2*)(&A[N * lda]);
  std::iota(ipiv, &ipiv[N], 1);
  ...
  for (int32_t i = 0; i < N; ++i) {
    ...
    internal::Cholesky::imax_double2_complex(stream, N - i, Aprev, &diag[i], &A[i * (lda + 1)], lda, pivot_i, scale);
    cudaStreamSynchronize(stream);
...
int32_t device::Cholesky::complex_quad_float_potrfp(cudaStream_t stream, int32_t N, complex_float4* A, int32_t lda, int32_t* ipiv) {
  int32_t algnN = (N + 3) & (~3), *pivot_i = &ipiv[algnN + 4];
  float4* scale = (float4*)(&ipiv[algnN]), *diag = (float4*)(&A[N * lda]);
  std::iota(ipiv, &ipiv[N], 1);
  ...
  for (int32_t i = 0; i < N; ++i) {
    ...
    internal::Cholesky::imax_float4_complex(stream, N - i, Aprev, &diag[i], &A[i * (lda + 1)], lda, pivot_i, scale);
    cudaStreamSynchronize(stream);
