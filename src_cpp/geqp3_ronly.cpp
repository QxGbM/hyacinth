...
int32_t device::QR::dgeqp3_ronly(cudaStream_t stream, cublasHandle_t handle, geqp3_params params, double* A, int32_t lda, int32_t* ipiv, void* workspace) {
  ...
  internal::int8::r8i_TN_gemm_stridedA(stream, handle, N, params.iter_k, algnN, algnM, iA, iA, orderA, AHA, orderC);
  ...
  if (elem_bytes == 4) {
    ...
    internal::int8::decode_dd_strided_i32(stream, order_lo, order_hi, N, mat_dd, AHA, algnN);
    ...
    internal::int8::decode_qf_strided_i32(stream, order_lo, order_hi, N, mat_qf, AHA, algnN);
...
int32_t device::QR::zgeqp3_ronly(cudaStream_t stream, cublasHandle_t handle, geqp3_params params, std::complex<double>* A, int32_t lda, int32_t* ipiv, void* workspace) {
  ...
  internal::int8::r8i_TN_gemm_stridedA(stream, handle, N, params.iter_k, algnN, algnM, iA, iA, orderA, AHA, orderC);
  internal::int8::r8i_TN_gemm_stridedA(stream, handle, N, params.iter_k, algnN, algnM, A_im, A_im, orderA, AHA, orderC);
  internal::int8::r8i_TN_gemm_stridedA(stream, handle, N, params.iter_k, algnN, algnM, iA, A_im, orderA, AHA_im, orderC);
  ...
  if (elem_bytes == 8) {
    ...
    double2* mat_dd = (double2*)mat;
    internal::int8::decode_dd_strided_i32(stream, order_lo, order_hi, N, mat_dd, AHA, algnN);
    internal::int8::decode_dd_strided_i32(stream, order_lo, order_hi, N, &mat_dd[strideC], AHA_im, algnN);
    ...
    float4* mat_qf = (float4*)mat;
    internal::int8::decode_qf_strided_i32(stream, order_lo, order_hi, N, mat_qf, AHA, algnN);
    internal::int8::decode_qf_strided_i32(stream, order_lo, order_hi, N, &mat_qf[strideC], AHA_im, algnN);
