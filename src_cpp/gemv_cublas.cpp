...
void internal::Cholesky::gemv_cublas_f64(cudaStream_t stream, cublasHandle_t handle, double* scale, int32_t j, int32_t M, int32_t N, double* A, int32_t lda, double* D, double* diag_piv) {
  if (2 <= M) {
    ...
    gemv_pp_f64(stream, j, N, M, scale[0], A, lda, D, diag_piv);
  }
  else if (1 == M)
    cudaMemcpyAsync(&A[N], scale, sizeof(double), cudaMemcpyHostToDevice, stream);
}

void internal::Cholesky::gemv_cublas_f32(cudaStream_t stream, cublasHandle_t handle, float* scale, int32_t j, int32_t M, int32_t N, float* A, int32_t lda, float* D, float* diag_piv) {
  if (2 <= M) {
    ...
    gemv_pp_f32(stream, j, N, M, scale[0], A, lda, D, diag_piv);
  }
  else if (1 == M)
    cudaMemcpyAsync(&A[N], scale, sizeof(float), cudaMemcpyHostToDevice, stream);
}

void internal::Cholesky::gemv_cublas_cf64(cudaStream_t stream, cublasHandle_t handle, double* scale, int32_t j, int32_t M, int32_t N, std::complex<double>* A, int32_t lda, double* D, double* diag_piv) {
  if (2 <= M) {
    ...
    gemv_pp_cf64(stream, j, N, M, scale[0], A, lda, D, diag_piv);
  }
  else if (1 == M) {
    scale[1] = 0.;
...
void internal::Cholesky::gemv_cublas_cf32(cudaStream_t stream, cublasHandle_t handle, float* scale, int32_t j, int32_t M, int32_t N, std::complex<float>* A, int32_t lda, float* D, float* diag_piv) {
  if (2 <= M) {
    ...
    gemv_pp_cf32(stream, j, N, M, scale[0], A, lda, D, diag_piv);
  }
  else if (1 == M) {
    scale[1] = 0.f;
