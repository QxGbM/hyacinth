...
inline void igemm_dispatcher(cudaStream_t stream, cublasHandle_t handle, int32_t M, int32_t N, hyacinPrecision_t Atype, const void* A, int32_t lda, 
  int32_t umax, const int32_t* vexp, int32_t orderA, int32_t orderC, uint64_t* acc, hyacinAlgorithm_t alg) {
  if (alg == HYACIN_ALG_LIMBS) switch(Atype) {
    case HYACIN_F64: internal::int8::i63AHA_limbs(stream, handle, M, N, orderA, (const double*)A, lda, umax, vexp, orderC, acc); return;
    case HYACIN_F32: internal::int8::i63AHA_limbs(stream, handle, M, N, orderA, (const float*)A, lda, umax, vexp, orderC, acc); return;
    case HYACIN_F16: internal::int8::i63AHA_limbs(stream, handle, M, N, orderA, (const __half*)A, lda, umax, vexp, orderC, acc); return;
    case HYACIN_F64_COMPLEX: internal::int8::i63AHA_limbs(stream, handle, M, N, orderA, (const cuDoubleComplex*)A, lda, umax, vexp, orderC, acc); return;
    case HYACIN_F32_COMPLEX: internal::int8::i63AHA_limbs(stream, handle, M, N, orderA, (const cuComplex*)A, lda, umax, vexp, orderC, acc); return;
    case HYACIN_F16_COMPLEX: internal::int8::i63AHA_limbs(stream, handle, M, N, orderA, (const __half2*)A, lda, umax, vexp, orderC, acc); return;
    default: return;
  }
...
extern "C" void hyacinXsyherk1Drow(hyacinHandle_t handle, int32_t localM, int32_t globalM, int32_t N, int32_t umax, hyacinPrecision_t Atype, const void* A, int32_t lda, hyacinPrecision_t Gtype, void* G, int32_t ldg, hyacinAlgorithm_t alg, ncclComm_t col_comm) {
  ...
  if (cudaSuccess != cudaMallocAsync((void**)&vexp, uint64_t(N) * sizeof(int32_t), handle.cudaStream))
    throw std::runtime_error("Workspace allocation failed at Integer SY/HERK.");
  vexp_dispatcher(handle.cudaStream, localM, N, Atype, A, lda, vexp);
  ...
  int64_t stride = int64_t(N) * int64_t(N + 1) / int64_t(2);
  hyacinXAllReduce1Drow(handle, orderC, Complex, stride, tp, col_comm);
