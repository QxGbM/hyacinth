...
const int32_t limbs_threshold = 30; // umax < 30 : Limbs, 30 <= umax : CRT
...
inline hyacinPrecision_t real_precision(hyacinPrecision_t prec) {
  return hyacinPrecision_t(int32_t(prec) & 7);
}

inline hyacinPrecision_t complex_precision(hyacinPrecision_t prec) {
  return hyacinPrecision_t((int32_t(prec) & 7) | 8);
}
...
extern "C" void hyacinXcpqrk_autoTune(double epi, int32_t M, int32_t u_extra, int32_t* umax, hyacinPrecision_t Atype, hyacinPrecision_t* ComputeType, hyacinAlgorithm_t* alg) {
  ...
  hyacinPrecision_t precA = real_precision(Atype);
  double machine_epi = precA == HYACIN_F32 ? double(std::numeric_limits<float>::epsilon()) : epi_dd;
  ...
  int32_t Complex = int32_t(Atype != precA), use_limbs = int32_t(u < limbs_threshold);
