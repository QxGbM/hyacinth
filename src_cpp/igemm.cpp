...
void device::MixPrecAHA::rATA(cudaStream_t stream, cublasHandle_t handle, int32_t M, int32_t N, int32_t algnN, int32_t umax, const void* A, int32_t lda, Precision precA, void* C, Precision precC) {
  ...
  if (precA == Precision::FP64) {
    ...
    internal::int8::quantize_f64(stream, M, N, (const double*)A, lda, umax, (uint64_t*)v_exp, iA, algnM);
  }
    ...
    internal::int8::quantize_f32(stream, M, N, (const float*)A, lda, umax, (uint64_t*)v_exp, iA, algnM);
  }

  internal::int8::i8GemmT(stream, handle, N, algnN, algnM, iA, orderA, 0, (uint64_t*)acc, orderC, (int32_t*)workspace);
...
void device::MixPrecAHA::cAHA(cudaStream_t stream, cublasHandle_t handle, int32_t M, int32_t N, int32_t algnN, int32_t umax, const void* A, int32_t lda, Precision precA, void* C, Precision precC) {
  ...
  if (precA == Precision::FP64) {
    ...
    internal::int8::quantize_cf64(stream, M, N, (const std::complex<double>*)A, lda, umax, (uint64_t*)v_exp, iA, algnM);
  }
  else if (precA == Precision::FP32) {
    ...
    internal::int8::quantize_cf32(stream, M, N, (const std::complex<float>*)A, lda, umax, (uint64_t*)v_exp, iA, algnM);
  }

  internal::int8::i8GemmT(stream, handle, N, algnN, algnM, iA, orderA, 0, (uint64_t*)acc, orderC, (int32_t*)workspace);
  internal::int8::i8GemmT(stream, handle, N, algnN, algnM, iA_imag, orderA, 1, (uint64_t*)acc, orderC, (int32_t*)workspace);
  internal::int8::i8GemmF(stream, handle, N, algnN, algnM, iA, iA_imag, orderA, 0, (uint64_t*)acc_imag, orderC, (int32_t*)workspace);
