...
void device::MixPrecAHA::igemm_params(double* epi, int32_t N, int32_t* algnN, int32_t* umax, Precision precA, Precision* precC) {
  ...
  *umax += int32_t(std::ceil(-std::log2(*epi)));
  if (*umax <= umax_threshold)
    *umax = ((*umax + 9) & (~7)) - 2;
}

inline std::tuple<int32_t, int32_t, int32_t, int32_t, int64_t, int64_t, int64_t, int64_t> i8gemm_ext_params(int32_t M, int32_t N, int32_t algnN, int32_t umax, int32_t Complex, device::Precision prec) {
  int32_t algorithm = int32_t(umax <= umax_threshold);
  int32_t algnM = (M + 255) & (~255);
  int32_t orderA = algorithm ? ((umax + 9) >> 3) : 8;
  ...
  int32_t orderC = algorithm ? (1 + (bits / 63)) : (1 + ((n_moduli << 3) / 63));
...
void device::MixPrecAHA::igemm_workspace(int32_t M, int32_t N, int32_t algnN, int32_t umax, int32_t Complex, Precision precC, int64_t* workspace) {
  int32_t algnM, orderA, orderC, n_moduli; int64_t i8_bytes, scratch_bytes, acc_bytes, vec_bytes;
  std::tie(algnM, orderA, orderC, n_moduli, i8_bytes, scratch_bytes, acc_bytes, vec_bytes) = i8gemm_ext_params(M, N, algnN, umax, Complex, precC);
...
void device::MixPrecAHA::rATA(cudaStream_t stream, cublasHandle_t handle, int32_t M, int32_t N, int32_t algnN, int32_t umax, const void* A, int32_t lda, Precision precA, void* C, Precision precC) {
  int32_t algnM, orderA, orderC, n_moduli; int64_t i8_bytes, scratch_bytes, acc_bytes, vec_bytes;
  std::tie(algnM, orderA, orderC, n_moduli, i8_bytes, scratch_bytes, acc_bytes, vec_bytes) = i8gemm_ext_params(M, N, algnN, umax, 0, precC);
...
void device::MixPrecAHA::cAHA(cudaStream_t stream, cublasHandle_t handle, int32_t M, int32_t N, int32_t algnN, int32_t umax, const void* A, int32_t lda, Precision precA, void* C, Precision precC) {
  int32_t algnM, orderA, orderC, n_moduli; int64_t i8_bytes, scratch_bytes, acc_bytes, vec_bytes;
  std::tie(algnM, orderA, orderC, n_moduli, i8_bytes, scratch_bytes, acc_bytes, vec_bytes) = i8gemm_ext_params(M, N, algnN, umax, 1, precC);
