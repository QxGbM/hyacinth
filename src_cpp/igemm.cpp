...
void device::MixPrecAHA::igemm_params(double epi, int32_t M, int32_t u_extra, int32_t* umax, Precision Atype, Precision* ComputeType, Algorithm* alg) {
  ...
  Precision precA = real_precision(Atype);
  double machine_epi = precA == Precision::FP32 ? double(std::numeric_limits<float>::epsilon()) : epi_dd;
  ...
  int32_t Complex = int32_t(Atype != precA), use_limbs = int32_t(u < umax_threshold);
