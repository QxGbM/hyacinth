...
inline void matcopy(cublasHandle_t handle, int32_t M, int32_t N, const double* A, int32_t lda, double* B, int32_t ldb)
{ double one = 1., zero = 0.; cublasDgeam(handle, CUBLAS_OP_N, CUBLAS_OP_N, M, N, &one, A, lda, &zero, B, ldb, B, ldb); }
inline void matcopy(cublasHandle_t handle, int32_t M, int32_t N, const float* A, int32_t lda, float* B, int32_t ldb)
{ float one = 1.f, zero = 0.f; cublasSgeam(handle, CUBLAS_OP_N, CUBLAS_OP_N, M, N, &one, A, lda, &zero, B, ldb, B, ldb); }
inline void matcopy(cublasHandle_t handle, int32_t M, int32_t N, const cuDoubleComplex* A, int32_t lda, cuDoubleComplex* B, int32_t ldb)
...
inline void matcopy(cublasHandle_t handle, int32_t M, int32_t N, const cuComplex* A, int32_t lda, cuComplex* B, int32_t ldb)
...
template <class complex_t> inline cublasOperation_t adjoint_op();
template <> inline cublasOperation_t adjoint_op<double>() { return CUBLAS_OP_T; }
template <> inline cublasOperation_t adjoint_op<float>() { return CUBLAS_OP_T; }
template <> inline cublasOperation_t adjoint_op<cuDoubleComplex>() { return CUBLAS_OP_C; }
template <> inline cublasOperation_t adjoint_op<cuComplex>() { return CUBLAS_OP_C; }
...
inline void ax_transform(cublasHandle_t handle, char transb, int32_t M, int32_t N, int32_t K, complex_t* A, int32_t lda, const complex_t* X, int32_t ldx, void* dev_work, uint64_t dev_work_bytes) {
  ...
  cublasOperation_t op = (transb == 'T' || transb == 't' || transb == 'C' || transb == 'c') ? adjoint_op<complex_t>() : CUBLAS_OP_N;
  for (int32_t i = 0; i < M; i += rows) {
    ...
    matcopy(handle, m, K, (const complex_t*)dev_work, ld, &A[i], lda);
  }
}
...
extern "C" void hyacinXtransform(cublasHandle_t handle, char transb, int32_t M, int32_t N, int32_t K, hyacinPrecision_t AXtype, void* A, int32_t lda, const void* X, int32_t ldx, void* dev_work, uint64_t dev_work_bytes) {
  ...
  switch(AXtype) {
    case HYACIN_F64:
      ax_transform(handle, transb, M, N, K, (double*)A, lda, (const double*)X, ldx, dev_work, dev_work_bytes); break;
    case HYACIN_F32:
      ax_transform(handle, transb, M, N, K, (float*)A, lda, (const float*)X, ldx, dev_work, dev_work_bytes); break;
    case HYACIN_F64_COMPLEX:
      ax_transform(handle, transb, M, N, K, (cuDoubleComplex*)A, lda, (const cuDoubleComplex*)X, ldx, dev_work, dev_work_bytes); break;
    case HYACIN_F32_COMPLEX:
      ax_transform(handle, transb, M, N, K, (cuComplex*)A, lda, (const cuComplex*)X, ldx, dev_work, dev_work_bytes); break;
    default: return;
  }
}
