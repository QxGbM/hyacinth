...
inline void gemm_accumulate_crt(cudaStream_t stream, cublasHandle_t handle, int32_t M, int32_t N, int32_t K, int32_t moduli, const int8_t* AT, const int8_t* A, 
  ...
  if (K <= iter_k) {
    ...
    internal::int8::accumulate_remainder_i32tensor(stream, accum | (last << 1), strideC, n_moduli, iter, workspace, strideC, C, strideACC);
  }
  else {
    int32_t rem = K & (iter_k - 1); rem = rem < iter_h ? (rem + iter_k) : rem;
    ...
    for (int32_t k = 0; k < range_k; k += iter_k) {
      ...
      cublasGemmStridedBatchedEx(handle, CUBLAS_OP_T, CUBLAS_OP_N, M, N, iter_k, &one, AT_k, CUDA_R_8I, K, strideA, AN_k, CUDA_R_8I, K, strideA,
        &zero, workspace, CUDA_R_32I, M, strideC, moduli, CUBLAS_COMPUTE_32I, CUBLAS_GEMM_DEFAULT);
      internal::int8::accumulate_remainder_i32tensor(stream, k == 0 ? accum : 1, strideC, n_moduli, iter, workspace, strideC, C, strideACC);
    }
    ...
    if (iter_h < rem) {
      cublasGemmStridedBatchedEx(handle, CUBLAS_OP_T, CUBLAS_OP_N, M, N, iter_h, &one, AT_k, CUDA_R_8I, K, strideA, AN_k, CUDA_R_8I, K, strideA,
        &zero, workspace, CUDA_R_32I, M, strideC, moduli, CUBLAS_COMPUTE_32I, CUBLAS_GEMM_DEFAULT);
      internal::int8::accumulate_remainder_i32tensor(stream, range_k == 0 ? accum : 1, strideC, n_moduli, iter, workspace, strideC, C, strideACC);
      cublasGemmStridedBatchedEx(handle, CUBLAS_OP_T, CUBLAS_OP_N, M, N, rem - iter_h, &one, &AT_k[iter_h], CUDA_R_8I, K, strideA, &AN_k[iter_h], CUDA_R_8I, K, strideA,
        &zero, workspace, CUDA_R_32I, M, strideC, moduli, CUBLAS_COMPUTE_32I, CUBLAS_GEMM_DEFAULT);
      internal::int8::accumulate_remainder_i32tensor(stream, 1 | (last << 1), strideC, n_moduli, iter, workspace, strideC, C, strideACC);
    }
    else {
      cublasGemmStridedBatchedEx(handle, CUBLAS_OP_T, CUBLAS_OP_N, M, N, rem, &one, AT_k, CUDA_R_8I, K, strideA, AN_k, CUDA_R_8I, K, strideA,
        &zero, workspace, CUDA_R_32I, M, strideC, moduli, CUBLAS_COMPUTE_32I, CUBLAS_GEMM_DEFAULT);
      internal::int8::accumulate_remainder_i32tensor(stream, (range_k == 0 ? accum : 1) | (last << 1), strideC, n_moduli, iter, workspace, strideC, C, strideACC);
    }
  }
}

void internal::int8::i63ATA_f64_crt(cudaStream_t stream, cublasHandle_t handle, int32_t M, int32_t N, const double* A, int32_t lda, int32_t umax, const int32_t* vec_expon, int32_t algnM, int32_t n_moduli, int32_t orderC, uint64_t* C, int32_t ldc, int8_t* workspace) {
...
void internal::int8::i63ATA_f32_crt(cudaStream_t stream, cublasHandle_t handle, int32_t M, int32_t N, const float* A, int32_t lda, int32_t umax, const int32_t* vec_expon, int32_t algnM, int32_t n_moduli, int32_t orderC, uint64_t* C, int32_t ldc, int8_t* workspace) {
  int64_t strideA = int64_t(algnM) * int64_t(N), strideC = int64_t(ldc) * int64_t(N + 1);
  int32_t* scratch = (int32_t*)&workspace[strideA << 3];
...
void internal::int8::i63AHA_cf64_crt(cudaStream_t stream, cublasHandle_t handle, int32_t M, int32_t N, const std::complex<double>* A, int32_t lda, int32_t umax, const int32_t* vec_expon, int32_t algnM, int32_t n_moduli, int32_t orderC, uint64_t* C, int32_t ldc, int8_t* workspace) {
  ...
  for (int32_t i = 0; (i << 3) < n_moduli; ++i) {
    ...
    gemm_accumulate_crt(stream, handle, ldc, N, algnM, moduli, workspace, &workspace[stride], n_moduli, i, accum, last, &C[strideIm], scratch);
  }
  vsum_cf64(stream, M, N, A, lda, umax, vec_expon, orderC, &C[strideC - int64_t(ldc)], strideC);
}

void internal::int8::i63AHA_cf32_crt(cudaStream_t stream, cublasHandle_t handle, int32_t M, int32_t N, const std::complex<float>* A, int32_t lda, int32_t umax, const int32_t* vec_expon, int32_t algnM, int32_t n_moduli, int32_t orderC, uint64_t* C, int32_t ldc, int8_t* workspace) {
  int64_t strideA = int64_t(algnM) * int64_t(N), strideC = int64_t(ldc) * int64_t(N + 1), strideIm = strideC * int64_t(orderC);
  int32_t* scratch = (int32_t*)&workspace[strideA << 4];
