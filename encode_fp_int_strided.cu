...
__global__ void encode_fp_strided_i8(int32_t order, int32_t M, int32_t N, real_const_ptr A, int32_t lda, const int32_t* __restrict__ vec_expon, uint32_t* __restrict__ inA, int32_t ldi, int32_t strideI) {
  ...
  for (int32_t col = blockIdx.y; col < N; col += GRID_X) {
    ...
    for (int32_t i = block_offset; i < M1; i += elements) {
      ...
      for (int32_t k = 0; k < order; ++k)
        block_store.Store(&inA_i[k * strideI], *(uint32_t(*)[1])(&threadB[k]));
    }
    ...
    if (0 < M2 && blockIdx.x == 0) {
      ...
      uint32_t* inA_i = &inA[(M1 >> COMPLEX) + col * ldi];
      int32_t len_M2 = (M2 + items - 1) >> COMPLEX;
      for (int32_t k = 0; k < order; ++k)
        block_store.Store(&inA_i[k * strideI], *(uint32_t(*)[1])(&threadB[k]), len_M2);
    }
  }
}
