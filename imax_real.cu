...
__global__ void update_reduce_real(int32_t N, real_const_ptr A, real_ptr X, real_ptr C, int32_t ldc, int32_t* i_out, real_ptr rsq_out) {
  ...
  for (int32_t i = 0; i < N; i += elements) {
    ...
    block_load.Load(&A[i], thread_A, num_items, real_t());
    block_load.Load(&X[i], thread_X, num_items, real_t());
...
void internal::Cholesky::imax_update_double(cudaStream_t stream, int32_t N, const double* A, double* X, double* C, int32_t ldc, int32_t* piv, double* rsq) {
...
void internal::Cholesky::imax_update_float(cudaStream_t stream, int32_t N, const float* A, float* X, float* C, int32_t ldc, int32_t* piv, float* rsq) {
...
void internal::Cholesky::imax_update_double2(cudaStream_t stream, int32_t N, const double2* A, double2* X, double2* C, int32_t ldc, int32_t* piv, double2* rsq) {
...
void internal::Cholesky::imax_update_float4(cudaStream_t stream, int32_t N, const float4* A, float4* X, float4* C, int32_t ldc, int32_t* piv, float4* rsq) {
...
void internal::Cholesky::imax_double(cudaStream_t stream, int32_t N, double* X, double* C, int32_t ldc, int32_t* piv, double* rsq) {
  constexpr int32_t items_per_thread = thread_bytes / sizeof(double);
  reduce_real <double, double* __restrict__, block_threads, items_per_thread>
    <<< 1, block_threads, 0, stream >>> (N, X, C, ldc, piv, rsq);
}

void internal::Cholesky::imax_float(cudaStream_t stream, int32_t N, float* X, float* C, int32_t ldc, int32_t* piv, float* rsq) {
  constexpr int32_t items_per_thread = thread_bytes / sizeof(float);
  reduce_real <float, float* __restrict__, block_threads, items_per_thread>
    <<< 1, block_threads, 0, stream >>> (N, X, C, ldc, piv, rsq);
}

void internal::Cholesky::imax_double2(cudaStream_t stream, int32_t N, double2* X, double2* C, int32_t ldc, int32_t* piv, double2* rsq) {
  constexpr int32_t items_per_thread = thread_bytes / sizeof(double2);
  reduce_real <double2, double2* __restrict__, block_threads, items_per_thread>
    <<< 1, block_threads, 0, stream >>> (N, X, C, ldc, piv, rsq);
}

void internal::Cholesky::imax_float4(cudaStream_t stream, int32_t N, float4* X, float4* C, int32_t ldc, int32_t* piv, float4* rsq) {
  constexpr int32_t items_per_thread = thread_bytes / sizeof(float4);
