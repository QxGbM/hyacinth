...
int32_t geqp3_ronly(cublasHandle_t handle, double epi, int32_t rank, int32_t M, int32_t N, const double* A, int32_t lda, int32_t* jpiv, double* R, int32_t ldr) {
  ...
  int32_t p = 0;
  rank = hyacinXcpqrk(handle, 'R', epi, M, N, rank, p, umax, HYACIN_F64, A, lda, (int32_t*)piv, HYACIN_F64, R, ldr, precC, dev_work, pinned_work, alg);
...
int32_t main(int32_t argc, char* argv[]) {
  ...
  double rel_err = check_answer_dlra(rank, M, N, matA.data(), M, ipiv.data(), matX.data(), N);
