...
double check_answer(int32_t rank, int32_t M, int32_t N, const double* A, int32_t lda, const int32_t* jpiv, const double* X, int32_t ldx) {
  if (rank <= 0)
    return std::numeric_limits<double>::quiet_NaN();

  std::vector<double> matB(int64_t(M) * int64_t(N)), matC(int64_t(M) * int64_t(rank));
  ...
  cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, M, N, rank, -1., &matC[0], M, X, ldx, 1., &matB[0], M);
...
int32_t main(int32_t argc, char* argv[]) {
  ...
  device::interp_decomp_f64(handle, epi, N, M, N, d_A, M, ipiv.data(), d_X, N);
  std::fill(ipiv.begin(), ipiv.end(), 0);
  cudaMemcpy(d_A, matA.data(), M * N * sizeof(double), cudaMemcpyHostToDevice);

  cudaEventRecord(start, stream);
  int32_t rank = device::interp_decomp_f64(handle, epi, N, M, N, d_A, M, ipiv.data(), d_X, N);
