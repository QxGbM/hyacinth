...
void make_1D_oscilatory(double w, int32_t M, int32_t N, float* A, int32_t lda) {
...
int32_t main(int32_t argc, char* argv[]) {
  ...
  if (M <= 2048 && N <= 2048) {
    std::vector<float> matB(M * N), matC(M * rank), matX(N * N);
    ...
    cblas_sgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, M, N, rank, 1., &matC[0], M, &matX[0], N, 0., &matB[0], M);
