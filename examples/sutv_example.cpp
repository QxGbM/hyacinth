...
double check_answer(int32_t M, int32_t N, int32_t rank, const float* U, int32_t ldu, const float* V, int32_t ldv, const float* B, int32_t ldb) {
  ...
  cblas_sgemm(CblasColMajor, CblasNoTrans, CblasConjTrans, M, N, rank, 1.f, U, ldu, V, ldv, 0.f, &matQ[0], M);
...
int32_t utv_factorize(cudaStream_t stream, cublasHandle_t cublasH, cusolverDnHandle_t cusolverH, double epi, int32_t M, int32_t N, const float* A, int32_t lda, float* UT, int32_t ldu, float* V, int32_t ldv) {
  ...
  hyacinXutvk_bufferSize(cusolverH, params, epi, N, rank, N, HYACIN_F32, &dev_work_bytes_new, &pinned_work_bytes_new);
  ...
  rank = hyacinXutvk(cublasH, cusolverH, params, epi, M, N, rank, p, A, lda, V, ldv, UT, ldu, HYACIN_F32, dev_work_bytes_new, dev_work, pinned_work_bytes_new, pinned_work);
...
int32_t main(int32_t argc, char* argv[]) {
  ...
  float* d_A = nullptr, *d_U = nullptr, *d_V = nullptr;
  ...
  cudaMemcpy(matU.data(), d_U, M * N * sizeof(float), cudaMemcpyDeviceToHost);
