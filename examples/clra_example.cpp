...
double check_answer(int32_t rank, int32_t M, int32_t N, const std::complex<float>* A, int32_t lda, const int32_t* jpiv, std::complex<float>* R, int32_t ldr) {
  if (rank <= 0)
    return std::numeric_limits<double>::quiet_NaN();

  std::complex<float> one(1.f, 0.f), minus_one(-1.f, 0.f), zero(0.f, 0.f);
  cblas_ctrsm(CblasColMajor, CblasLeft, CblasUpper, CblasNoTrans, CblasNonUnit, rank, N - rank, &one, R, ldr, &R[int64_t(rank) * int64_t(ldr)], ldr);
  LAPACKE_claset(LAPACK_COL_MAJOR, 'A', rank, rank, zero, one, R, ldr);

  std::vector<std::complex<float>> matB(int64_t(M) * int64_t(N)), matC(int64_t(M) * int64_t(rank)), matR(int64_t(N) * int64_t(rank));
  LAPACKE_clacpy(LAPACK_COL_MAJOR, 'A', M, N, A, lda, &matB[0], M);
  for (int32_t i = 0; i < N; ++i) {
    if (i < rank)
      cblas_ccopy(M, &matB[int64_t(jpiv[i] - 1) * int64_t(M)], 1, &matC[int64_t(i) * int64_t(M)], 1);
    cblas_ccopy(rank, &R[int64_t(i) * int64_t(ldr)], 1, &matR[int64_t(jpiv[i] - 1) * int64_t(rank)], 1);
  }

  double nrm = cblas_scnrm2(matB.size(), &matB[0], 1);
  cblas_cgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, M, N, rank, &minus_one, &matC[0], M, &matR[0], rank, &one, &matB[0], M);
  double err = cblas_scnrm2(matB.size(), &matB[0], 1);
  return err / nrm;
}
