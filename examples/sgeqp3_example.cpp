...
int32_t main(int32_t argc, char* argv[]) {
  ...
  device::sgeqp3_ronly(cublasH, cusolverH, params, epi, M, N, d_A, M, ipiv.data(), d_tau);
  std::fill(ipiv.begin(), ipiv.end(), 0);
  cudaMemcpy(d_A, matA.data(), M * N * sizeof(float), cudaMemcpyHostToDevice);

  cudaEventRecord(start, stream);
  int32_t ret = device::sgeqp3_ronly(cublasH, cusolverH, params, epi, M, N, d_A, M, ipiv.data(), d_tau);
